...
from models import User, UserActivityInfo, ChannelInfo
...
def test__ActivityProcessingService_process_channel__no_activities_and_users__new_notification_for_users(mocker,
                                                                                                         service):
    users = [
...
def test__ActivityProcessingService_process_channel__fresh_activity_and_user__no_new_notifaction(mocker, service):
    ...
    activity = [
        UserActivityInfo(
            id=1,
            last_seen_timestamp=int(dt.timestamp())
        )
    ]
...
def test__ActivityProcessingService_get_channel_new_activity_notifications__old_channel_info_contains_activity__no_channel_notification(
    ...
    channel_info = ChannelInfo(timestamp=channel_info_timestamp, activities=[
        UserActivityInfo(id=1, last_seen_timestamp=channel_info_timestamp)
    ], channel_id=1)
